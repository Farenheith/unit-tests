...
function test(iterable) {
    return __asyncGenerator(this, arguments, function* test_1() {
        for (const str of iterable) {
            yield yield __await(yield __await(new Promise((resolve) => resolve(console.log(str)))));
        }
    });
}
async function getValue(code) {
    var e_1, _a;
    try {
        for (var _b = __asyncValues(test(code)), _c; _c = await _b.next(), !_c.done;) {
            const item = _c.value;
            console.log(item);
        }
    }
